...
def run():
    ...
    for line in lines:
        ...
        # Match question start like **Q1. text** or **Question 1:**
        q_m1 = re.match(r'\*\*(?:Q|Question)\s*\d+[.:]?\s*(.*?)\*\*', line, re.IGNORECASE)
        q_m2 = re.match(r'\*\*(?:Q|Question)\s*\d+[.:]?\*\*', line, re.IGNORECASE) # Question text is on next line
        
        if q_m1:
            save_q()
            q_text = q_m1.group(1).strip()
        ...
        if state == 'EXPLANATIONS':
            exp_m = re.search(r'\*?\s*\*\*([A-D])\)\s*(?:True|False|is correct|is incorrect)\.\*\*\s*(.*)', line, re.IGNORECASE)
            if exp_m:
